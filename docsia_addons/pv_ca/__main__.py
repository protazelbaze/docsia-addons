...
def main():
    ...
    r = sub.add_parser("run"); r.add_argument("--apply", action="store_true"); r.add_argument("--run-id"); r.add_argument("--trigger", default="manual"); r.add_argument("--limit", type=int); r.add_argument("--page", choices=["current", "archives", "all"], default="all")
    ...
    if a.cmd == "initdb":
        ...
        from . import discover as disc
        page_urls = {"current": [disc.DEFAULT_CURRENT],
                     "archives": [disc.DEFAULT_ARCHIVES],
                     "all": None}[a.page]
        run(a.run_id or datetime.now().strftime("%Y%m%d_%H%M%S"),
            trigger=a.trigger, apply=a.apply, limit=a.limit, page_urls=page_urls)
    elif a.cmd == "retry":
